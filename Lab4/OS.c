...
unsigned int flag_os = 0;//for os_time_ms
struct tcb{
...
int OS_AddPeriodicThread(void(*task)(void), unsigned long period, unsigned long priority) {
  ...
  TIMER1_ICR_R = 0x00000001;    // 5) clear TIMER1A timeout flag_os
...
void Timer1A_Handler(void) {
	/*timer1_count++;
	TIMER1_ICR_R = 0x00000001;	// clear TIMER1A timeout flag_os
...
int OS_AddPeriodicThread2(void(*task)(void), unsigned long period, unsigned long priority) {
  ...
  TIMER3_ICR_R = 0x00000001;    // 5) clear TIMER1A timeout flag_os
...
void Timer3A_Handler(void) {
	/*timer3_count++;
	TIMER3_ICR_R = 0x00000001;	// clear TIMER3A timeout flag_os
...
void Timer2A_Init(void){
  ...
  TIMER2_ICR_R = 0x00000001;       // 6) clear timer1A timeout flag_os
...
{ int32_t status;
  ...
  status = StartCritical();
	...
	if(head == NULL){
	  ...
	  if((prev==curr)&&(flag_os == 0)){//insert before the head
			tcbs[j].next = head;
			head = &tcbs[j];
		}
		else {
...
int OS_AddSW1Task(void(*task)(void), unsigned long priority){
  ...
  GPIO_PORTF_AMSEL_R &= ~0x10; //disable analog on PF1
	...
	GPIO_PORTF_ICR_R = 0x10; //clear flag_os4
...
int OS_AddSW2Task(void(*task)(void), unsigned long priority){
  ...
  GPIO_PORTF_AMSEL_R &= ~0x01; //disable analog on PF0
	...
	GPIO_PORTF_ICR_R = 0x01; //clear flag_os0
  ...
  GPIO_PORTF_IS_R &= ~0x01;     // (d) PF0 is edge-sensitive
	GPIO_PORTF_IBE_R |= 0x01; //PF0 is both edges
  GPIO_PORTF_ICR_R = 0x01;      // (e) clear flag_os0
